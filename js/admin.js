  ...
  stopLeadsListener = onSnapshot(q, snap => {
    ...
    adminLeads.innerHTML = leads.map(l => `
      <article class="admin-item" data-lead-card="${escapeAttr(l.id)}">
        ...
        <div class="admin-actions"><button class="primary-btn save-lead-note" data-id="${escapeAttr(l.id)}">Save Note</button>${l.commissionComplete&&!l.commissionPaid?`<button class="ghost-btn mark-commission" data-id="${escapeAttr(l.id)}">Paid</button>`:' '}${l.commissionPaid?'<span class="admin-active-badge active">Paid</span>':''}</div>
      </article>`).join("");
