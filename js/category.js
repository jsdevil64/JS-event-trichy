...
const categoryNames = { decoration:"Decoration", photography:"Photography", catering:"Catering", mahal:"Mahal", "event-planner":"Event Planner" };
const categoryDescriptions = {
  decoration:"Find decoration providers for your event.", photography:"Find photographers and videographers.",
  catering:"Find catering services and food packages.", mahal:"Find event halls and marriage mahals.", "event-planner":"Find event planners to organize and manage your special events."
};
...
function cardHtml(p){
  return `<article class="service-card">
    <div class="service-card-head"><div class="service-icon">${category==='decoration'?'🎈':category==='photography'?'📸':category==='catering'?'🍽️':category==='mahal'?'🏛️':'🎉'}</div><div><h3>${escapeHtml(p.brandName||'Provider')}</h3><p>${escapeHtml(p.location||'')}</p></div></div>
